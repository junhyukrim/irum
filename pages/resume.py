        ...
        for idx, i in enumerate(st.session_state.intro_data):
            ...
            with cols[0]:
                selected_category = st.selectbox(
                    ...
                    index=list(intro_topic_map.keys()).index(
                        personal_info.get(f'intro_category_{i}', '성장 배경 및 가치관')
                    ),
                    key=f"intro_category_{i}"
                )

            with cols[1]:
                topic_list = intro_topic_map.get(selected_category, [])
                selected_topic = st.selectbox(
                    "주제",
                    topic_list,
                    index=topic_list.index(
                        personal_info.get(f'intro_topic_{i}', topic_list[0])
                    ) if personal_info.get(f'intro_topic_{i}') in topic_list else 0,
